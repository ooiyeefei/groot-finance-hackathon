...
export default defineSchema({
  ...
  line_items: defineTable({
    legacyId: v.optional(v.string()),
    accountingEntryId: v.string(),  // Legacy — may contain stale references post-migration
    itemDescription: v.string(),
    quantity: v.number(),
    unitPrice: v.number(),
    totalAmount: v.number(),
  ...
  expense_claims: defineTable({
    ...
    // Linked Accounting Entry (legacy — may contain stale references post-migration)
  ...
  einvoice_request_logs: defineTable({
    ...
    reconDescription: v.optional(v.string()),            // Recon step output (form field descriptions)
    generatedHint: v.optional(v.string()),               // cuaHint text generated by troubleshooter
    hintEffectivenessOutcome: v.optional(v.union(        // Did the hint help on next attempt?
      v.literal("helped"),
      v.literal("not_helped"),
      v.literal("pending"),
    )),
    confidenceGateScore: v.optional(v.number()),         // Tier 1 confidence prediction (0-1)
    confidenceGateDecision: v.optional(v.union(          // Tier 1 gate outcome
      v.literal("proceed"),
      v.literal("skip"),
    )),
    failureCategory: v.optional(v.union(                 // Classified failure type
    ...
    perFieldResults: v.optional(v.array(v.object({       // Per-field fill outcomes
      fieldName: v.string(),
      filled: v.boolean(),
      selector: v.optional(v.string()),
      error: v.optional(v.string()),
    }))),
    buyerProfileMatchResult: v.optional(v.object({       // Buyer profile selection result
      profileSelected: v.optional(v.string()),
      reasoning: v.optional(v.string()),
      matchType: v.optional(v.string()),                 // "tin_exact" | "fuzzy_name" | "recency" | "create_new"
    })),
    dspyModuleVersion: v.optional(v.string()),           // Version ID of optimized module used
  })
  ...
  vendor_price_history: defineTable({
    ...
    accountingEntryId: v.optional(v.string()),  // Legacy — may contain stale references
