/**
 * PO Match DSPy Optimization — weekly batch optimization via MIPROv2
 *
 ...
 * 2. Minimum diversity: ≥10 unique item descriptions in corrections (prevents overfitting)
 * 3. New data only: skips if no new corrections since last optimization
 *
 * Follows the same pattern as bankReconOptimization.ts.
 * Key difference: uses po_match_corrections table and optimize_po_matching_model MCP tool.
 */
...
export const getBusinessesReadyForOptimization = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    for (const c of allCorrections) {
      ...
      existing.count++;
      // Use both PO and invoice line descriptions for diversity check
      existing.uniqueDescriptions.add(c.originalPoLineDescription.toLowerCase().trim());
      existing.uniqueDescriptions.add(c.originalInvoiceLineDescription.toLowerCase().trim());
    ...
    for (const [bizId, data] of businessData) {
      ...
      // For PO matching, we use platform="po_matching_{businessId}" in dspy_model_versions
      const platformKey = `po_matching_${bizId}`;
    ...
    for (const skip of skippedBusinesses) {
      console.log(`[PO Match DSPy] Skipping business ${skip.businessId}: ${skip.reason}`);
    }
  ...
  handler: async (ctx, args) => {
    ...
    return corrections
      ...
      .map((c) => ({
        ...
        originalPoLineDescription: c.originalPoLineDescription,
        originalInvoiceLineDescription: c.originalInvoiceLineDescription,
...
/**
 * Get latest active model for a business (po_matching domain).
 */
export const getLatestModel = internalQuery({
  args: { businessId: v.string() },
  handler: async (ctx, args) => {
    const platformKey = `po_matching_${args.businessId}`;
...
export const triggerOptimization = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const corrections = await ctx.runQuery(
      _internal.functions.poMatchOptimization.getAllCorrectionsForBusiness,
      { businessId: args.businessId }
    );

    if (corrections.length < MIN_CORRECTIONS_FOR_OPTIMIZATION) {
      console.log(`[PO Match DSPy] Skipping business ${args.businessId}: only ${corrections.length} corrections (need ${MIN_CORRECTIONS_FOR_OPTIMIZATION})`);
      return;
    }

    // 2. Get current active model
    const activeModel = await ctx.runQuery(
      _internal.functions.poMatchOptimization.getLatestModel,
      { businessId: args.businessId }
    );

    // 3. Call optimize_po_matching_model on DSPy Lambda via MCP
    ...
    try {
      const result = await callMCPTool<OptimizeResult>({
        toolName: "optimize_po_matching_model",
        businessId: args.businessId,
        args: {
          corrections,
      ...
      if (!result) {
        console.error(`[PO Match DSPy] Optimization returned null for business ${args.businessId}`);
        return;
      }
      ...
      if (result.success && result.newModelS3Key) {
        ...
        await ctx.runMutation(
          _internal.functions.poMatchOptimization.markOptimizationConsumed,
          {
            businessId: args.businessId,
        ...
        console.log(
          `[PO Match DSPy] Optimization completed for business ${args.businessId}: ${result.beforeAccuracy} -> ${result.afterAccuracy} (${result.durationMs}ms, consumed up to ${args.latestCorrectionId})`
        );
      } else {
        console.warn(`[PO Match DSPy] Optimization did not improve for business ${args.businessId}: ${result.errorMessage}`);
      }
    } catch (error) {
      console.error(`[PO Match DSPy] Optimization failed for business ${args.businessId}:`, error);
    }
  },
});
...
export const weeklyOptimization = internalAction({
  ...
  handler: async (ctx, args) => {
    const businesses = await ctx.runQuery(
      _internal.functions.poMatchOptimization.getBusinessesReadyForOptimization as any,
      { force: args.force ?? false }
    );

    console.log(`[PO Match DSPy] Weekly optimization: ${businesses.length} businesses ready`);

    for (const biz of businesses as Array<{ businessId: string; latestCorrectionId: string; reason: string }>) {
      console.log(`[PO Match DSPy] Optimizing business ${biz.businessId}: ${biz.reason}`);
      try {
        await ctx.runAction(
          _internal.functions.poMatchOptimization.triggerOptimization as any,
          { businessId: biz.businessId, latestCorrectionId: biz.latestCorrectionId }
        );
      } catch (error) {
        console.error(`[PO Match DSPy] Failed to optimize business ${biz.businessId}:`, error);
      }
    }
  },
});
...
/**
 * Manual optimization — admin-triggered for a specific business.
 * Requires authentication. Forces optimization regardless of safeguards.
 */
export const manualOptimize = internalMutation({
  args: {
    businessId: v.string(),
  },
  handler: async (ctx, args) => {
    // Get latest correction to use as checkpoint
    const corrections = await ctx.db
      .query("po_match_corrections")
    ...
    if (corrections.length === 0) {
      throw new Error(`No PO match corrections found for business ${args.businessId}`);
    }

    // Find latest correction ID
    ...
    // Schedule the optimization action (mutations can't call actions directly)
    await ctx.scheduler.runAfter(0, _internal.functions.poMatchOptimization.triggerOptimization, {
      businessId: args.businessId,
      latestCorrectionId,
    });
