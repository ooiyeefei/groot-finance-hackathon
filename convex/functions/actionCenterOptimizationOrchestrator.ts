 ...
 *
 * Separated from actionCenterOptimization.ts to avoid circular type inference
 * (TypeScript can't resolve self-referencing internalAction types in the same file).
 */
...
export const runForAllBusinesses = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const business of businesses) {
      try {
        const result = await ctx.runAction(internal.functions.actionCenterOptimization.prepareAndRun, {
