...
export default function COGSCategoryFormModal({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.gl_account.trim()) {
      errors.gl_account = 'GL account is required'
    }
        ...
        <div className="relative transform overflow-hidden rounded-xl bg-gray-800 shadow-2xl text-left transition-all w-full max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <h4 className="text-sm font-medium text-white">Accounting Information</h4>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="gl_account" className="text-white">GL Account *</Label>
                    <Input
                      id="gl_account"
                      ...
                      disabled={isLoading}
                    />
                    {validationErrors.gl_account && (
                      <p className="text-red-400 text-xs mt-1">{validationErrors.gl_account}</p>
                    )}
                  </div>
