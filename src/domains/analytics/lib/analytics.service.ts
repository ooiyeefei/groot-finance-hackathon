...
/**
 * Get real-time dashboard metrics using optimized RPC function
...
export async function getRealtimeMetrics(
  ...
  console.log('[Analytics Service] getRealtimeMetrics called')
  console.log('[Analytics Service] Date range:', startDate, 'to', endDate)
  ...
  const rpcParameters = {
    ...
    p_end_date: endDate,
    user_id_param: userProfile.id  // Pass Supabase UUID to RPC
  }

  console.log('[Analytics Service] Calling RPC: get_dashboard_analytics_realtime with params:', rpcParameters)

  const { data: rpcResult, error: rpcError } = await supabase
    .rpc('get_dashboard_analytics_realtime', rpcParameters)

  if (rpcError) {
    console.error('[Analytics Service] RPC error:', rpcError)
    throw new Error(`Failed to fetch realtime analytics: ${rpcError.message}`)
  }

  // Return empty analytics for no data
  if (!rpcResult) {
    console.log('[Analytics Service] No data returned, returning empty analytics')
    return {
  ...
  console.log('[Analytics Service] RPC completed successfully')

  // Transform RPC result to FinancialAnalytics interface
