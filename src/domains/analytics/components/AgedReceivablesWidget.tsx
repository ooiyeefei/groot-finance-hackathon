...
export default function AgedReceivablesWidget({
  ...
  const getRiskIcon = () => {
    ...
    if (totalOutstanding === 0) return <DollarSign className="w-5 h-5 text-gray-400" />;
    if (averageRiskScore > 75 || highRiskTransactions > 0) return <AlertTriangle className="w-5 h-5 text-red-400" />;
    if (averageRiskScore > 50) return <TrendingUp className="w-5 h-5 text-yellow-400" />;
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div>
              ...
              <span className={`ml-2 font-medium ${
                ...
                agedReceivables.average_risk_score > 25 ? 'text-orange-400' : 'text-green-400'
              }`}>
