...
import { TransactionType } from '@/domains/accounting-entries/types'
...
export default function CategorySelector({
  ...
  const getFilteredCategories = () => {
    ...
    } else if (accountingEntryType === 'Cost of Goods Sold') {
  ...
  const handleCategorySelect = async (newCategory: string) => {
    setIsLoading(true)
    try {
      const response = await fetch(`/api/v1/accounting-entries/${accountingEntryId}/category`, {
        method: 'PATCH',
        headers: {
