...
export default function MatchCandidatesSheet({
  ...
  const filteredCandidates = candidates?.filter((c) => {
    ...
    return (
      ...
      c.accountingEntry.referenceNumber?.toLowerCase().includes(q) ||
      c.accountingEntry.vendorName?.toLowerCase().includes(q) ||
      c.accountingEntry.category?.toLowerCase().includes(q)
    )
  })

  const handleManualMatch = async (entryId: Id<'accounting_entries'>) => {
  ...
  const toggleSplitSelection = (entryId: Id<'accounting_entries'>) => {
  ...
  const selectedTotal = filteredCandidates
    ...
    .reduce((sum, c) => sum + c.accountingEntry.originalAmount, 0) ?? 0
  ...
  const handleSplitMatch = async () => {
    ...
    await createSplitMatch({
      bankTransactionId,
      accountingEntryIds: selectedEntryIds,
    })
    onClose()
  }
      ...
      <SheetContent className="w-[640px] sm:max-w-[640px] overflow-y-auto">
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <button
            ...
            title="Split matching: match one bank transaction to multiple accounting entries"
          >
