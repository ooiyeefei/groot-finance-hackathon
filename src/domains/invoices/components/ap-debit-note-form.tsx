...
import { useAPDebitNoteMutation } from '@/domains/invoices/hooks/use-ap-adjustments'
...
export function APDebitNoteForm({
  invoiceId,
  businessId,
  currency,
  onClose,
  onSuccess,
}: APDebitNoteFormProps) {
  const { createDebitNote } = useAPDebitNoteMutation()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to create AP debit note')
    } finally {
    ...
    <Card className="bg-card border-border">
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Record additional charges from supplier (e.g., freight surcharge, price adjustment).
        </p>
        ...
        <div className="space-y-1.5">
          ...
          <Input
            placeholder="e.g., Freight surcharge, price adjustment..."
            value={reason}
