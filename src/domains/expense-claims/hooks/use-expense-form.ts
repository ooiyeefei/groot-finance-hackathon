...
export function useExpenseForm(props: UseExpenseFormProps): UseExpenseFormReturn {
  ...
  const [formData, setFormData] = useState<ExpenseFormData>({
    ...
    expense_category: '',
    original_amount: 0,
    original_currency: userHomeCurrency || 'MYR',
    home_currency: userHomeCurrency || 'MYR',
    transaction_date: '',
    vendor_name: '',
    reference_number: '',
    notes: '',
  ...
  const loadExpenseClaim = useCallback(async () => {
    ...
    try {
      ...
      setFormData({
        ...
        original_currency: claim.transaction?.original_currency || claim.currency || userHomeCurrency || 'MYR',
        home_currency: claim.transaction?.home_currency || userHomeCurrency || 'MYR',
  ...
  const initializeFormData = useCallback(async () => {
    if (mode === 'create' && extractionResult) {
      ...
      setFormData({
        ...
        original_currency: extractionResult.extractedData.currency || userHomeCurrency || 'MYR',
        home_currency: userHomeCurrency || 'MYR',
  ...
  return {
    ...
    categoriesLoading,
    categoriesError,
    userHomeCurrency: userHomeCurrency || 'MYR',
