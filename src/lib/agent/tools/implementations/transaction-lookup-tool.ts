/**
 * Secure Transaction Lookup Tool - SQL with RLS Enforcement
...
export class TransactionLookupTool extends BaseTool {
  ...
  protected async executeInternal(parameters: ToolParameters, userContext: UserContext): Promise<ToolResult> {
    ...
    try {
      this.secureLog(`Processing transaction lookup query: ${query}`, userContext)
      ...
      if (error) {
        this.secureLog(`Transaction query error: ${error.message}`, userContext, 'error')
        return {
          success: false,
      ...
      this.secureLog(`Transaction lookup completed: ${transactions.length} results`, userContext)
      ...
      this.secureLog(`Transaction lookup execution error: ${error}`, userContext, 'error')
      return {
        success: false,
  ...
  protected async checkUserPermissions(userContext: UserContext): Promise<{ authorized: boolean; reason?: string }> {
    ...
    try {
      ...
      if (error || !userProfile) {
        this.secureLog(`User profile check failed: ${error?.message}`, userContext, 'error')
        return {
          authorized: false,
          reason: 'User profile validation failed'
        }
      }
