...
function createVercelOidcCredentialProvider(
  ...
  return async () => {
    ...
    const provider = fromWebToken({
      ...
      roleSessionName: `finanseal-${Date.now()}`,
...
// Maps old Supabase storage patterns to S3
// =============================================================================

/**
 * Map old Supabase bucket names to S3 prefixes
 */
const SUPABASE_BUCKET_TO_S3_PREFIX: Record<string, S3Prefix> = {
  invoices: 'invoices',
  expense_claims: 'expense_claims',
...
/**
 * Get presigned download URL using old Supabase-style bucket name
 * For backward compatibility during migration
 */
export async function getSignedUrlLegacy(
  bucketName: string,
  path: string,
  expiresIn: number = URL_EXPIRY.download
): Promise<{ signedUrl: string | null; error: string | null }> {
  const prefix = SUPABASE_BUCKET_TO_S3_PREFIX[bucketName]

  if (!prefix) {
    return { signedUrl: null, error: `Unknown bucket: ${bucketName}` }
  }

  try {
    const url = await getPresignedDownloadUrl(prefix, path, expiresIn)
...
/**
 * Upload file using old Supabase-style bucket name
 * For backward compatibility during migration
 */
export async function uploadFileLegacy(
  ...
  if (!prefix) {
    return { data: null, error: `Unknown bucket: ${bucketName}` }
  }
  ...
  const result = await uploadFile(prefix, path, file, contentType)
...
/**
 * Delete file using old Supabase-style bucket name
 */
export async function deleteFileLegacy(
  ...
  if (!prefix) {
    return { error: `Unknown bucket: ${bucketName}` }
  }

  const errors: string[] = []

  for (const path of paths) {
    const result = await deleteFile(prefix, path)
...
/**
 * List files using old Supabase-style bucket name
 */
export async function listFilesLegacy(
  ...
  data: Array<{ name: string; id: string; metadata: { size: number } }> | null
  error: string | null
}> {
  const prefix = SUPABASE_BUCKET_TO_S3_PREFIX[bucketName]
  ...
  const result = await listFiles(prefix, path, { maxKeys: options?.limit || 100 })
  ...
  // Map to Supabase-style response format
