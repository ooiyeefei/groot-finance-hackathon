 ...
 * Fetches expense claims dashboard data from the API endpoint
 */
...
export function useExpenseClaims(): UseExpenseClaimsReturn {
  ...
  // Reprocess claim mutation
  const reprocessMutation = useMutation({
    mutationFn: async (claimId: string): Promise<ExpenseClaimResponse> => {
      const response = await fetch(`/api/expense-claims/${claimId}/process`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      const result = await response.json();
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'Failed to reprocess claim');
      }
